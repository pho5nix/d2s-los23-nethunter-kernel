...
**Resolution:** Skip KSU 0.9.5. Use KernelSU-Next.
...
**Resolution:** Skip manual hooks. Use KernelSU-Next legacy-susfs-v2.
...
**Resolution:** Use `sidex15/KernelSU-Next` branch `legacy-susfs-v2` (commit `00cb93e`). This has hash `79e590113c4c4c0c222978e413a5faa801666957b1212a328e46c00c69821bf7` and size `0x3e6` — matching the v3.2.0 build 33129 manager APK.
...
**Resolution:** Switched to FreeRunner kernel which has both NetHunter (via `nethunter.config`) AND KSU-Next built in.
...
**Resolution:** Always pack `Image` (raw, ~50 MB) — never `Image.gz` or `Image.gz-dtb`. This is what `pack-boot-img.sh` does.
...
**Resolution:** Cosmetic warning only. The signature/hash check passes because `KSU_NEXT_MANAGER_HASH` and `_SIZE` are still pinned to v3.2.0 values in the Kbuild. Newer kernel build numbers within the same major version are forward-compatible.
...
**Resolution:** Skip the BusyBox app installer. Use KSU's bundled busybox:

```bash
adb shell "su -c 'mkdir -p /data/local/xbin && \
  ln -sf /data/adb/ksu/bin/busybox /data/local/xbin/busybox'"
```
...
If you do want metamodule support later (for, say, Play Integrity Fix module), install `meta-overlayfs` from KSU-Next module repo.
